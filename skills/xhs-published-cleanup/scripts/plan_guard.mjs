...
export const MODE = 'published-cleanup';
export const CONFIRMATION_PHRASE = 'CONFIRM XHS DESTRUCTIVE ACTIONS';
const ALLOWED_ACTIONS = new Set(['delete-note']);
...
export function validatePlan(plan, { requireConfirmed = false } = {}) {
  ...
  const protectedBoardIds = requireStringArray(plan.protectedBoardIds, 'protectedBoardIds', errors);
  if (protectedBoardIds.length > 0) errors.push('protectedBoardIds must be empty for published cleanup');
  ...
  if (!Array.isArray(plan.plannedActions)) {
    ...
    for (const [index, action] of plan.plannedActions.entries()) {
      ...
      if (protectedNoteIds.has(action.targetId)) errors.push(`${prefix} targets protected note ${action.targetId}`);
    }
  }
